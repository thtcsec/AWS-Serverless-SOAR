...
def lambda_handler(event, context):
    ...
    try:
        ...
        }  # Default for non-network findings
        intel_report = {}
        ...
        if remote_ip:
            ...
            if risk_data["decision"] == "IGNORE":
                ...
                return {
                    "statusCode": 200,
                    "body": f"Ignored due to low risk score: {risk_data['risk_score']}",
                }
...
def enforce_imdsv2(instance_id):
    ...
    try:
        ec2.modify_instance_metadata_options(InstanceId=instance_id, HttpTokens="required", HttpEndpoint="enabled")
    except Exception as e:
...
def notify_team(instance_id, finding_type, severity, risk_data=None, intel_report=None, approved=True):
    ...
    action_str = (
        "Instance Isolated, IAM Revoked, Snapshot Captured, STOPPED."
        if approved
        else "Waiting for Human-in-the-Loop approval via Slack/Jira."
    )
    ...
    sns.publish(TopicArn=SNS_TOPIC_ARN, Subject=f"AWS Security Response: {action_status}", Message=message)
