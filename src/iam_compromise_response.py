...
def lambda_handler(event, context):
    try:
        ...
        if not event_name or not event_name.startswith(
            ("Create", "Delete", "Update", "Attach", "Detach", "Add", "Remove")
        ):
...
def is_unusual_source_ip(source_ip, user_identity):
    ...
    try:
        ...
        response = cloudtrail.lookup_events(
            LookupAttributes=[{"AttributeKey": "Username", "AttributeValue": user_identity.get("userName", "")}],
...
def investigate_compromise(username, user_arn, event_name, source_ip):
    ...
    try:
        ...
        investigation_result = {
            ...
            "investigation_time": datetime.now(UTC).isoformat(),
        }
