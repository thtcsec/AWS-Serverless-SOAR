...
class CrossAccountResponder:
            ...
            for env, cfg in parsed.items():
                if isinstance(cfg, dict):
                    account_configs[env] = {
                        ...
                        "role_name": str(cfg.get("role_name", "soar-cross-account-responder")).strip()
                        or "soar-cross-account-responder",
                    }
    ...
    def assume_cross_account_role(self, account_name):
        ...
        try:
            ...
            response = self.sts_client.assume_role(
                RoleArn=role_arn,
                RoleSessionName=f"soar-cross-account-{account_name}-{int(datetime.now(UTC).timestamp())}",
                ...
                TagSpecifications=[
                    {
                        ...
                        "Tags": [
                            {
                                "Key": "Name",
                                "Value": f"forensic-{volume_id}-{int(datetime.now(UTC).timestamp())}",
                            },
                            {"Key": "Purpose", "Value": "cross-account-forensics"},
                            {"Key": "CreatedBy", "Value": "SOAR-CrossAccount"},
                            {"Key": "CreatedDate", "Value": datetime.now(UTC).isoformat()},
                        ],
                    }
                ],
            )
    ...
    def revoke_iam_credentials_cross_account(self, account_name, user_name, access_key_id=None):
        ...
        try:
            ...
            logger.info(
                f"Successfully revoked {len(revoked_keys)} access keys for user {user_name} in account {account_name}"
            )
            return result
...
def lambda_handler(event, context):
    ...
    try:
        ...
        logger.info("Cross-account operation completed successfully")
        return result
